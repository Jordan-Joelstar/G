  ...
  async (message, args) => {
    try {
      ...
      if (url.toLowerCase().includes("https")) {
        ...
        return await message.send("`MILES UPDATE WAS SUCCESSFULL ✅`");
      } else {
...
${menuThemeCommandPrefix} *𝙻𝙸𝚅𝙴 𝚃𝙸𝙼𝙴:* ${currentTime}
${menuThemeCommandPrefix} *𝙳𝙰𝚃𝙴:* ${currentDate}
${menuThemeCommandPrefix} *𝙿𝙻𝚄𝙶𝙸𝙽𝚂:* ${commands.length}
${menuThemeCommandPrefix} *𝚄𝚂𝙰𝙶𝙴 𝚃𝚁𝙴𝙽𝙳:* ${trend_usage}
${menuThemeCommandPrefix} *𝙳𝙱 𝙸𝙽𝙵𝙾:* ${database_info}
${menuThemeFooter}                         
*🕸𝑀⃪𝐼⃪𝐿⃪𝐸⃪𝑆⃪ 𝑀⃪𝐸⃪𝑁⃪𝑈⃪🕷*
${readmore}`;
...
astro_patch.cmd(
  {
    pattern: "setcmd",
    desc: "Stick a command to a sticker",
    category: "user",
    fromMe: true,
    filename: __filename,
  },
    ...
    try {
      if (!_0x291296) {
        return await _0x5d887.send(
          "*_Please provide cmd name by replying a Sticker. E.g setcmd kick(replied sticker)_*"
        );
      }
      ...
      if (_0x8e739e) {
        ...
        return await _0x5d887.send(
          '*_Cmd "' +
            ...
            '"._*\n*_These names are reset, If bot restarts_*'
        );
      } else {
        return await _0x5d887.send(
          "*_Provided Cmd( " +
    ...
    try {
      ...
      const designs = [
        ...
        async () => {
          ...
          const end = new Date().getTime();
          const pingSeconds = (end - start) / 1000;
          const captionText = `🕷𝑀⃪𝐼⃪𝐿⃪𝐸⃪𝑆⃪🕸 𝟸.𝟶.𝟶 ᴘᴀᴛᴄʜ\n\n*Ping:* ${pingSeconds} seconds\n\n*Fact:*\n${fact.result.fact}\n\n𝙼𝙸𝙻𝙴𝚂`;
...
smd(
  ...
  async (m) => {
    try {
      ...
      const message = `*𝙼𝙸𝙻𝙴𝚂 𝚄𝙿𝚃𝙸𝙼𝙴🥏:* ${uptimeHours}h ${uptimeMinutes}ᴍ ${uptimeSeconds}s\n \t_𝙴𝚇𝚃𝚁𝙰 𝙳𝙴𝚃𝙰𝙸𝙻𝚂_\n*𝚁𝙰𝙼 𝚄𝚂𝙰𝙶𝙴:* ${ramUsage.toFixed(2)} MB\n*𝙲𝙿𝚄 𝙽𝙰𝙼𝙴:* ${cpuModel}`;
      const contextInfo = {
        isForwarded: true,
        forwardingScore: 999,
        title: "𝙷𝙴𝙻𝙻𝙾 𝙱𝙾𝚂𝚂 ( ͡° ͜ʖ ͡°)",
        body: message,
        footerText: "🅼︎🅸🄻🅴🅂",
...
astro_patch.cmd(
  ...
  async (
    ...
    try {
      if (!_0x3b225e) {
        return _0x5a9ab6.reply("*𝔹𝕠𝕤𝕤 𝕡𝕣𝕠𝕧𝕚𝕕𝕖 𝕒 𝕢𝕦𝕖𝕣𝕪 𝕥𝕠 𝕣𝕦𝕟 𝕔𝕠𝕕𝕖*");
      }
