const {
  ...
  function _0x2953() {
    const _0x4f0c10 = [
      ...
      "Error While getting Ai response ",
      "url",
      "catch",
      "]&msg=[",
      "split",
  ...
  smd(
    ...
    async (message, input) => {
      try {
        ...
        return await message.send(`*𝙼𝙸𝙻𝙴𝚂:*\n ${result}`, { quoted: message });
      } catch (error) {
  ...
  smd({
    pattern: "gemini",
    category: "ai",
    desc: "Chat with gemini AI model",
    use: "<text>",
    filename: __filename,
  }, async (message, text, { cmdName }) => {
    if (!text) return message.reply(`*_Please provide a query_*\n*_Example ${prefix + cmdName} What is the meaning of life?_*`);
    ...
    async (_0x9bac01, _0x3700d4) => {
      try {
        ...
        try {
          ...
          console.log("ERROR IN IMAGINE RESPONSE FROM IMAGINE API n", _0x45726b);
        }
        if (
        ...
        await _0x9bac01.error(
          _0x5d8080 + "\n\ncommand: imagine",
          _0x5d8080,
          "*_No response from Server side, Sorry!!_*"
        );
      }
    }
  );
    ...
    async (_0x2acf30) => {
      try {
        if (
          ...
          if (
            _0x2acf30.reply_text.includes("𝙼𝙸𝙻𝙴𝚂 • ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
            _0x2dfb59[0].includes("𝙼𝙸𝙻𝙴𝚂 • ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
...
`;
 ...
 smd(
   ...
   async (m, url) => {
     try {
       if (!url) {
         return await m.send("*_provide a URL!_*");
       }
       ...
       if (!result || !result.medias || !result.medias.length) {
         return await m.send("*_No media found boss!_*");
       }
     ...
     try {
       ...
       if (!_0x1c586e) {
         return await _0x1ae8f8.reply(
           "*Provide Repo Url, _.gitclone https://github.com/Jordan-Joelstar/Miles_*"
         );
       }
       const _0x5906ab =
 ...
 smd({
   ...
   category: "downloader",
   filename: __filename,
   use: "<Hii,this is Miles>"
 }, async (_0x55aba2, _0x56da6b) => {
   try {
     let _0x204f81 = _0x55aba2.reply_text ? _0x55aba2.reply_text : _0x56da6b;
     if (!_0x204f81) {
       return _0x55aba2.reply("*_Example : .tts Hi,I am Miles, Your friendly neighborhood._*");
     }
     try {
