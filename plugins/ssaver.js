...
smd({
     ...
     async( message, match) => { 
       try{
         if (!match) return message.reply('Give a package name')
...
smd({
        ...
        async(message) => {
          ...
          }catch(e){return await message.error(`${e}\n\n command: couplepp`,e,`*_No results found, no vex!_*`) }
        }
...
smd({
    ...
    async(message, text) => {
 if(!text) return await message.reply('Give Me Number without + sign. Example: .iswa 2349054039xx')
        var inputnumber = text.split(" ")[0]
        if (!inputnumber.includes('x')) return message.reply(`*You did not add x*\nExample: iswa 2349054039xx  \n ${Config.caption}`)
...
let nobio = `\n*『 𝙰𝙲𝙲𝙾𝚄𝙽𝚃𝚂 𝙾𝙽 𝚆𝙷𝙰𝚃𝚂𝙰𝙿𝙿 𝚆𝙸𝚃𝙷 𝙽𝙾 𝙱𝙸𝙾』* \n`;
 let nobios='';
